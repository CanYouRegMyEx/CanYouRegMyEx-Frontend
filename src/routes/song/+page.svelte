...
let showRomanji = false;
let songData = null;
let error = null;

const unsubscribe = page.subscribe(($page) => {
  songData = $page.data?.songData || null;
  error = $page.data?.error || null;
});

function hasCast() { return songData?.cast && songData.cast.length > 0; }
function hasOpeningSpeech() {
  return (
    songData?.openingSpeech &&
    (songData.openingSpeech.japanese?.content || songData.openingSpeech.english?.content)
  );
}
function hasLyrics() { return (songData?.lyrics) || (songData?.lyricsRomanji); }
function hasTrackList() { return (songData?.trackList && songData.trackList.length > 0) || (songData?.cdCollection && songData.cdCollection.length > 0); }
</script>
